...
import com.bakery.api.modules.partner.services.PurchaseOrderService;
import com.bakery.api.modules.partner.services.PurchaseOrderService.*;
    ...
    public List<Map<String, Object>> getUnpaidOrders() {
        return purchaseOrderRepository
                .findAllByPaymentStatusNot(
                        com.bakery.api.framework.enums.PaymentStatus.PAID)
                .stream()
                .map(po -> {
                    Map<String, Object> m = new LinkedHashMap<>();
                    m.put("id",            po.getId());
    ...
    public List<Map<String, Object>> getLots(@RequestParam String ingredientCode) {
        ...
        return stockLotRepository
                .findAllByIngredientIdAndStatus(
                        ingredient.getId(),
                        com.bakery.api.framework.enums.StockLotStatus.AVAILABLE)
                .stream()
                .map(lot -> {
                    Map<String, Object> m = new LinkedHashMap<>();

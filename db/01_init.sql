...
CREATE TABLE IF NOT EXISTS ingredient_stock (
    ...
    -- Ngày kiểm đếm phụ kiện gần nhất tại branch (NULL = chưa kiểm lần nào)
...
ALTER TABLE ingredient_stock ADD COLUMN IF NOT EXISTS last_reconcile_date DATE;
...
CREATE TABLE IF NOT EXISTS goods_transfer (
    ...
    -- Traceability: lô KHO_BEP ← phiếu TRF này ← lô KHO_TONG gốc
...
-- Safety net: đảm bảo cột tồn tại kể cả khi bảng sót lại từ schema cũ
ALTER TABLE goods_transfer ADD COLUMN IF NOT EXISTS source_transfer_id UUID REFERENCES goods_transfer(id);
...
-- Đảm bảo cột tồn tại kể cả khi bảng được tạo từ schema cũ không có entity_type
ALTER TABLE command_request ADD COLUMN IF NOT EXISTS entity_type VARCHAR(100) NOT NULL DEFAULT 'UNKNOWN';
ALTER TABLE command_request ADD COLUMN IF NOT EXISTS entity_id   UUID;
...
ALTER TABLE entity_revision_log ADD COLUMN IF NOT EXISTS entity_type VARCHAR(100) NOT NULL DEFAULT 'UNKNOWN';
ALTER TABLE entity_revision_log ADD COLUMN IF NOT EXISTS entity_id   UUID;
...
ALTER TABLE activity_log ADD COLUMN IF NOT EXISTS entity_type VARCHAR(100) NOT NULL DEFAULT 'UNKNOWN';
...
-- ---------------------------------------------------------------------------
-- 21. INVENTORY LEDGER CORE (Single-Table Ledger Architecture V2)
-- ---------------------------------------------------------------------------

-- Lô tồn kho — mỗi row = 1 lô tại 1 branch
-- item_id: polymorphic FK → ingredient.id hoặc product.id (theo item_type)
CREATE TABLE IF NOT EXISTS inventory (
    ...
    source_tx_id  UUID,                      -- soft ref → inventory_transaction.id
    created_at    TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    updated_at    TIMESTAMPTZ    NOT NULL DEFAULT NOW(),
    CONSTRAINT chk_inv_item_type CHECK (item_type IN ('INGREDIENT', 'PRODUCT'))
);

CREATE INDEX IF NOT EXISTS idx_inv_branch    ON inventory(branch_id);
CREATE INDEX IF NOT EXISTS idx_inv_item      ON inventory(item_id, item_type);
CREATE INDEX IF NOT EXISTS idx_inv_fefo      ON inventory(branch_id, item_id, expiry_date ASC)
    WHERE qty_available > 0;

-- Phiếu kho — IMPORT | TRANSFER | ADJUSTMENT | WRITE_OFF | SALE | RETURN
CREATE TABLE IF NOT EXISTS inventory_transaction (
    id                   UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    code                 VARCHAR(30)   NOT NULL,
    transaction_type     VARCHAR(20)   NOT NULL,   -- TransactionType enum
    transaction_reason   VARCHAR(30)   NOT NULL,   -- TransactionReason enum
    status               VARCHAR(20)   NOT NULL DEFAULT 'PENDING',
    ...
    updated_by           VARCHAR(100),
    updated_at           TIMESTAMPTZ,
    -- BaseAdminEntity fields
    approved_by          VARCHAR(100),
    approved_at          TIMESTAMPTZ,
    entity_status        entity_status NOT NULL DEFAULT 'ACTIVE',
    CONSTRAINT uq_inv_tx_code UNIQUE (code),
    CONSTRAINT chk_inv_tx_type   CHECK (transaction_type   IN ('IMPORT','TRANSFER','ADJUSTMENT','WRITE_OFF','SALE','RETURN')),
...
CREATE INDEX IF NOT EXISTS idx_inv_tx_type   ON inventory_transaction(transaction_type, status);
CREATE INDEX IF NOT EXISTS idx_inv_tx_branch ON inventory_transaction(from_branch_id, to_branch_id);
...
CREATE TABLE IF NOT EXISTS inventory_transaction_line (
    ...
    lot_id          UUID          REFERENCES inventory(id),
    note            TEXT,
    CONSTRAINT chk_inv_txl_item_type CHECK (item_type IN ('INGREDIENT', 'PRODUCT'))
);

CREATE INDEX IF NOT EXISTS idx_inv_txl_tx   ON inventory_transaction_line(transaction_id);
CREATE INDEX IF NOT EXISTS idx_inv_txl_item ON inventory_transaction_line(item_id);

-- Thanh toán nhiều đợt (chỉ cho IMPORT)
...
CREATE INDEX IF NOT EXISTS idx_inv_txp_tx ON inventory_transaction_payment(transaction_id);

-- ---------------------------------------------------------------------------
-- 22. RECONCILIATION SOURCES (3-way: BEP → SHOP → POS)
-- ---------------------------------------------------------------------------

-- Dữ liệu bán hàng POS — upload từ file xuất POS hàng ngày
CREATE TABLE IF NOT EXISTS pos_sales_data (
    ...
    CONSTRAINT uq_pos_sales_data UNIQUE (sales_date, branch_id, item_id)
);

CREATE INDEX IF NOT EXISTS idx_psd_date   ON pos_sales_data(sales_date, branch_id);
CREATE INDEX IF NOT EXISTS idx_psd_item   ON pos_sales_data(item_id);

-- Báo cáo cuối ngày của shop — nhân viên submit mỗi tối
CREATE TABLE IF NOT EXISTS daily_shop_report (
    ...
    item_id                  UUID          NOT NULL,
    item_type                VARCHAR(20)   NOT NULL DEFAULT 'PRODUCT',
    qty_leftover_theoretical NUMERIC(10,4) NOT NULL,
    qty_destroyed_actual     NUMERIC(10,4) NOT NULL,
    submitted_by             VARCHAR(100)  NOT NULL,
    submitted_at             TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
    note                     TEXT,
    CONSTRAINT uq_daily_shop_report UNIQUE (report_date, branch_id, item_id)
);

CREATE INDEX IF NOT EXISTS idx_dsr_date ON daily_shop_report(report_date, branch_id);
CREATE INDEX IF NOT EXISTS idx_dsr_item ON daily_shop_report(item_id);

-- ---------------------------------------------------------------------------
-- 23. ACCESSORY STOCKTAKE LOG (kiểm đếm phụ kiện tại cửa hàng)
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS accessory_stocktake_log (
    id                 UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    branch_id          UUID          NOT NULL REFERENCES branch(id),
    ingredient_id      UUID          NOT NULL REFERENCES ingredient(id),
    ...
    qty_on_hand_before NUMERIC(18,4) NOT NULL,
    qty_pos_sold       NUMERIC(18,4) NOT NULL DEFAULT 0,
    qty_theoretical    NUMERIC(18,4) NOT NULL,
    qty_actual         NUMERIC(18,4) NOT NULL,
    qty_loss           NUMERIC(18,4) NOT NULL DEFAULT 0,
    qty_overage        NUMERIC(18,4) NOT NULL DEFAULT 0,
    adjustment_ref     VARCHAR(30),
    note               TEXT,
    created_by         VARCHAR(100)  NOT NULL,
    created_at         TIMESTAMPTZ   NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_asl_date       ON accessory_stocktake_log(stocktake_date, branch_id);
CREATE INDEX IF NOT EXISTS idx_asl_ingredient ON accessory_stocktake_log(ingredient_id);

-- ---------------------------------------------------------------------------
-- 24. RECONCILIATION VIEW (3-way: BEP → SHOP → POS)
-- ---------------------------------------------------------------------------
